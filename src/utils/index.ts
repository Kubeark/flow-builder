...
export const getRegisterNode = (registerNodes: IRegisterNode[], type?: string) => registerNodes.find((node) => type && node.type === type);

export const getIsStartNode = (registerNodes: IRegisterNode[], type?: string) => registerNodes.find((item) => item.type === type)?.isStart;

export const getIsEndNode = (registerNodes: IRegisterNode[], type?: string) => registerNodes.find((item) => item.type === type)?.isEnd;

export const getIsLoopNode = (registerNodes: IRegisterNode[], type?: string) => registerNodes.find((item) => item.type === type)?.isLoop;

export const getIsConditionNode = (registerNodes: IRegisterNode[], type?: string) => {
	const conditionNode = getRegisterNode(registerNodes, type);
	const branchNode = registerNodes.find((item) => type && item.conditionNodeType === type);

	return conditionNode && branchNode && branchNode?.type !== branchNode?.conditionNodeType;
};

export const getIsBranchNode = (registerNodes: IRegisterNode[], type?: string) => {
	const branchNode = getRegisterNode(registerNodes, type);
	const conditionNode = getRegisterNode(registerNodes, branchNode?.conditionNodeType);

	return branchNode && conditionNode && branchNode?.type !== branchNode?.conditionNodeType;
};

export const getAbstractNodeType: (registerNodes: IRegisterNode[], type?: string) => AbstractNodeType = (
...
export const createNewNode = (registerNodes: IRegisterNode[], type?: string, customCreateUuid = createUuid) => {
	...
	const extraProps: any = isBranchNode
		? {
				children: [
					createNewNode(registerNodes, registerNode.conditionNodeType, customCreateUuid),
					createNewNode(registerNodes, registerNode.conditionNodeType, customCreateUuid),
				],
				...initialNodeData,
		  }
...
const getNextNode = (node: INode, allNodes: INode[]) => {
	...
	const nextPath = [...path.slice(0, path.length - 1), String(Number(path[path.length - 1]) + 1)];

	return allNodes.find((item) => (item.path || []).join('-') === nextPath.join('-'));
};

const getNextIdsByBranchNode: (node: INode, allNodes: INode[], registerNodes: IRegisterNode[]) => string[] = (
	node,
	allNodes,
	registerNodes,
) => {
	...
	const branchNode = allNodes.find((item) => (item.path || []).join('-') === branchPath.join('-'));
...
const getNextIds = (node: INode, allNodes: INode[], registerNodes: IRegisterNode[]) => {
...
export const buildFlatNodes = (params: { registerNodes: IRegisterNode[]; nodes: INode[] }) => {
...
export const buildTreeNodes = (params: { nodes: INode[] }) => {
	...
	const cloneNodes = JSON.parse(JSON.stringify(nodes.map(({ next, ...node }) => node)));
...
export const loadRemoteNode = async (params: IRegisterRemoteNode) => {
	...
	const tasks = [url, cssUrl].filter(Boolean).map((item) => item && import(item));

	return new Promise<IRegisterNode>((resolve, reject) => {
		Promise.all(tasks)
			.then((res) => {
				if (res.length === 2) {
					document.adoptedStyleSheets = [...document.adoptedStyleSheets, res[1].default];
				}
...
export const exchangeNodes = (nodes: any[], startIndex: number, endIndex: number) => {
