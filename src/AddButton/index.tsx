...
import { getRegisterNode, getIsStartNode, getIsEndNode, getIsBranchNode, getIsConditionNode } from '../utils';
...
import './index.less';
import { IRegisterNode } from '../index';
...
const AddNodeButton: React.FC<IProps> = (props) => {
	...
	const droppable =
		...
		(Array.isArray(addableNodeTypes) ? addableNodeTypes.includes(dragType) : true);
	...
	const options = registerNodes.filter(
		(item: IRegisterNode) =>
			...
			(Array.isArray(addableNodeTypes) ? addableNodeTypes.includes(item.type) : true),
	);
	...
	const addableOptions = AddableComponent ? (
		...
		<>
			{options.map((item: IRegisterNode) => {
				const registerNode = getRegisterNode(registerNodes, item.type);
				const defaultIcon = getIsBranchNode(registerNodes, item.type) ? AddBranchIcon : AddNormalIcon;
				return (
					<div className="flow-builder-addable-node-item" key={item.type} onClick={() => handleAddNode(item.type)}>
						<span className="flow-builder-addable-node-icon">{registerNode?.addIcon || <img src={defaultIcon} />}</span>
